    ...
    public class PlaintextAsset<T>
    {
        ...
        private float ParseFloat(string str)
        {
            if (float.TryParse(str, out var tmp))
                return tmp;
            Debug.Logging.Log($"Could not successfully parse float from string '{str}'.", Debug.Logging.Severity.High);
            return 0;
        }
        ...
        private int ParseInt(string str)
        {
            if (int.TryParse(str, out var tmp))
                return tmp;
            Debug.Logging.Log($"Could not successfully parse int from string '{str}'.", Debug.Logging.Severity.High);
            return 0;
        }
        ...
        private uint ParseUnsignedInt(string str)
        {
            if (uint.TryParse(str, out var tmp))
                return tmp;
            Debug.Logging.Log($"Could not successfully parse unsigned int from string '{str}'.", Debug.Logging.Severity.High);
            return 0;
        }
                ...
                foreach (var field in typeof(T).GetFields())
                {
                    foreach (var attribute in field.GetCustomAttributes(typeof(TextAssetOpcodeAttribute), false))
                    {
                        if (((TextAssetOpcodeAttribute)attribute).opcodes.Contains(opcode))
                        {
                            ...
                            {
                                Debug.Logging.Log($"Unknown attribute type {field.FieldType.Name} on opcode {string.Join(" ", ((TextAssetOpcodeAttribute)attribute).opcodes)}.", Debug.Logging.Severity.High);
                            }
                        }
                    }
                }
