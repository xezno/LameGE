    ...
    public class PlaintextAsset<T>
    {
        ...
        private float ParseFloat(string str)
        {
            if (float.TryParse(str, out var tmp))
                return tmp;
            Logging.Log($"Could not successfully parse float from string '{str}'.", Logging.Severity.High);
            return 0;
        }
        ...
        private int ParseInt(string str)
        {
            if (int.TryParse(str, out var tmp))
                return tmp;
            Logging.Log($"Could not successfully parse int from string '{str}'.", Logging.Severity.High);
            return 0;
        }
        ...
        private uint ParseUnsignedInt(string str)
        {
            if (uint.TryParse(str, out var tmp))
                return tmp;
            Logging.Log($"Could not successfully parse unsigned int from string '{str}'.", Logging.Severity.High);
            return 0;
        }
